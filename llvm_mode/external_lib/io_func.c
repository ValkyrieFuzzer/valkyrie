...
size_t __dfsw_fread(void *buf, size_t size, size_t count, FILE *fd,
  ...
  if (is_fuzzing_ffd(fd)) {
    if (ret > 0) assign_taint_labels_exf(buf, offset, ret, count, size);
...
#endif
  if (is_fuzzing_fd(fd)) {
    if (ret > 0) assign_taint_labels_exf(buf, offset, ret, count, 1);
...
#endif
  if (is_fuzzing_ffd(fd)) {
    if (ret > 0) assign_taint_labels(*lineptr, offset, ret);
    *ret_label = __angora_get_sp_label(offset, 1);
  } else {
    *ret_label = 0;
  }
